...
def process_eldorado_data(create_records)
    invalid_sizes = []
    invalid_tmi = []
    ...
    base_url = "http://www.eldoradotire.com/Default.aspx"
    base_response = ""
    (1..5).each do |i|
        begin
    ...
    if base_response != '' # there was no error
        ...
        base_data.xpath('//div[contains(@class, "main-nav")]/div/a/@href').each do |category_url|
            # we now have a page that includes all the models for the given category
            category_response = ''
            (1..5).each do |i|
                begin
                    category_response = ReadData("http://www.eldoradotire.com/#{category_url}")
                    break
                rescue Exception => e 
                    puts "Error processing #{model_url}: #{e.to_s} - try again"
                end
            end
            if category_response != '' # there was no error
                category_data = Nokogiri::HTML(category_response.to_s)
                category_data.xpath('//div[@class="info"]').each do |model_info|
                    model_name = model_info.xpath('h3/text()').text().to_s
                    model_url = "http://www.eldoradotire.com" + model_info.xpath('span[@class="tire-info"]/a/@href').text().to_s
        ...
        if false
            ...
            manu = TireManufacturer.find_or_create_by_name('Eldorado')
...
namespace :ScrapeEldorado do
    desc "Create tire manufacturers data from Eldorado"
    task populate: :environment do
        process_eldorado_data(true)
    end

    task test: :environment do
        process_eldorado_data(false)
    end
end
