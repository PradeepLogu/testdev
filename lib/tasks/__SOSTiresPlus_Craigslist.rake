...
namespace :SOSTiresPlus do
    desc "Create tirelistings for SOS Tires Plus from Craigslist data"
    task populate: :environment do
        t = TireStore.find_by_name("SOS Tires Plus")

        offset = 0
        tot = 0

        feed = RSS::Parser.parse(open('http://atlanta.craigslist.org/search/sss?query=sostiresplus.com+used+tires&srchType=A&format=rss&s=' + offset.to_s).read, false)
